    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(globalGamesData.data)) {
          ...
          const activeBoomingGames = data.data.games.filter(
            (game: any) => game.maker === "booming-games" && game.status === 1,
          );

          const finalBoomingGames = activeBoomingGames.filter((boomingGame) =>
            activeGlobalGames.some(
              (globalGame) => globalGame.id === boomingGame.id,
            ),
          );

          setGames(finalBoomingGames);
        } else {
  ...
  const closeGameUrl = () => {
    if (isMounted) {
      ...
      setTimeout(() => {
        ...
        router.replace(`/games?idMachine=${idMachineFromURL}&provider=booming`, undefined, { shallow: true });
        setLoading(false);
      }, 2000);
    }
  };
                  ...
                  {filteredGames
                      ...
                      .map((game) => (
                        <div key={game.id} className="col-3 col-md-3">
                          <Link
                            href={`?idMachine=${idMachineFromURL}&provider=booming&gameId=${game.id}`}
