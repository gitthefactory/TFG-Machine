    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(globalGamesData.data)) {
          ...
          const activePopOkGames = data.data.games.filter((game: any) => game.maker === 'popok' && game.status === 1);

          const finalPopOkGames = activePopOkGames.filter(popokGame =>
            activeGlobalGames.some(globalGame => globalGame.id === popokGame.id)
          );

          setGames(finalPopOkGames);
        } else {
  ...
  const closeGameUrl = () => {
    if (isMounted) {
      ...
      setTimeout(() => {
        ...
        router.replace(`/games?idMachine=${idMachineFromURL}&provider=popok`, undefined, { shallow: true });
        setLoading(false);
      }, 2000);
    }
  };
              ...
              <div className="swiper-slide-content">
                {filteredGames.slice(pageIndex * 8, (pageIndex + 1) * 8).map((game) => (
                  <div key={game.id} className="col-3 col-md-3">
                    <Link href={`?idMachine=${idMachineFromURL}&provider=popOk&gameId=${game.id}`} onClick={() => handleGameClick(game)} shallow>
